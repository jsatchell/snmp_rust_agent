...
This project follows semver - but that means no guarantees at all pre v1! There are no changes in the internal APIs and traits in this release, so this is a patch version (0.2.1) rather than a minor version.
...
The agent is 100% safe Rust code.

The 0.2.0 release implemented (more or less) the transaction model for Set PDUs. This required a change to the OidKeeper trait, defined in src/keeper.rs. The sample handlers, and the Rust MIB compiler have been updated to follow this.
...
Additional authentication hashes from RFC7630 have been added - SHA-224 and SHA-256. The other two options (SHA-384 and SHA-512) use a bigger block size, which requires more intrusive code changes, and will come along later. The password file generator supports this change as well.
...
from the configuration file. It can generate lines for the alternate password hashes, up to SHA-512, even though the agent does not yet
support anything other than SHA-1, SHA2-224 and SHA-256.
...
already been written in src/usm.rs.

## Tools for stub generation

At present, there is only developing tooling to help implement an useful agent, but it is possible with some patience. There is a stub generator, written in Rust. There used to be a Python one too, but it is now obsolete and has been removed.
