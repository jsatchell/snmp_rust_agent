...
fn write_object_ids<A>(out: &mut A, object_ids: &[ObjectIdentity]) -> Result<(), Error>
where
    A: std::io::Write,
{
...
fn write_module_compliances<A>(out: &mut A, mod_comps: &[ModuleCompliance]) -> Result<(), Error>
where
    A: std::io::Write,
{
...
fn loader_imp<A>(src: &mut A, mib_files: Vec<String>) -> Result<(), Error>
where
    A: std::io::Write,
{
...
",
    ...
    #[test]
    fn test_write_stub_loader() {
        let mut src = Cursor::new(Vec::<u8>::new());
        let mib_files = vec!["first".to_string(), "second".to_string()];
        let res = loader_imp(&mut src, mib_files);
        assert!(res.is_ok());
    }
    ...
    fn test_gen_stubs() {
        ...
        entries.insert("a", entry_fixture().unwrap());
        //let mib_files = vec!["first".to_string(), "second".to_string()];
        let obj = object_id_fixture().unwrap(); // Checked #[test]
        let object_ids = vec![obj];
        let mod_comps = vec![mod_comp_fixture().unwrap()];
