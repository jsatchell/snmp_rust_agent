...
pub fn load_stub(oid_map: &mut OidMap) {
   ...
   // The next group is for OBJECT-IDENTITIES
    ...
    let oid_usm_h_m_a_c_m_d5_auth_protocol: ObjectIdentifier =
        ObjectIdentifier::new(&ARC_USM_H_M_A_C_M_D5_AUTH_PROTOCOL).unwrap();
    let oid_usm_h_m_a_c_s_h_a_auth_protocol: ObjectIdentifier =
        ObjectIdentifier::new(&ARC_USM_H_M_A_C_S_H_A_AUTH_PROTOCOL).unwrap();
    let oid_usm_no_priv_protocol: ObjectIdentifier =
        ObjectIdentifier::new(&ARC_USM_NO_PRIV_PROTOCOL).unwrap();
    let oid_usm_d_e_s_priv_protocol: ObjectIdentifier =
