...
fn new() -> Self {
   KeepSnmpTrapOID {
       scalar: ScalarMemOid::new(simple_from_vec(&[1, 3, 6, 1]), OType::ObjectId, Access::NotificationOnly),
}
}
}
...
fn new() -> Self {
   KeepSysObjectID {
       scalar: ScalarMemOid::new(simple_from_vec(&[1, 3, 6, 1]), OType::ObjectId, Access::ReadOnly),
}
}
}
...
fn new() -> Self {
   let base_oid: ObjectIdentifier =
       ObjectIdentifier::new(&ARC_SYS_OR_TABLE).unwrap();

   KeepSysORTable {
       table: TableMemOid::new(
         vec![vec![simple_from_int(4), simple_from_vec(&[1, 3, 6, 1]), simple_from_str(b"b"), ticks_from_int(0)]],
    vec![simple_from_int(4), simple_from_vec(&[1, 3, 6, 1]), simple_from_str(b"b"), ticks_from_int(0)],
    4,
    &base_oid,
...
pub fn load_stub(oid_map: &mut OidMap) {
    ...
    let oid_snmp_trap_oid: ObjectIdentifier =
        ObjectIdentifier::new(&ARC_SNMP_TRAP_OID).unwrap();
    let k_snmp_trap_oid: Box<dyn OidKeeper> = 
    ...
    let oid_sys_object_id: ObjectIdentifier =
        ObjectIdentifier::new(&ARC_SYS_OBJECT_ID).unwrap();
    let k_sys_object_id: Box<dyn OidKeeper> = 
       Box::new(KeepSysObjectID::new());
    oid_map.push(oid_sys_object_id, k_sys_object_id);
    let oid_sys_or_table: ObjectIdentifier =
        ObjectIdentifier::new(&ARC_SYS_OR_TABLE).unwrap();
    let k_sys_or_table: Box<dyn OidKeeper> = 
    ...
    let k_sys_or_last_change: Box<dyn OidKeeper> = 
    ...
    let k_snmp_in_asn_parse_errs: Box<dyn OidKeeper> = 
