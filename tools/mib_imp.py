...
def parse_obj_ident(text: str, obj_ids: dict):
    ...
    for i, part in enumerate(parts[1:]):           
        if "::=" in part: 
            ...
            oname = frags[-2] if frags[-1].isspace() else frags[-1]
            val = y.split("}")[0].strip() + " }"
            obj_ids[oname.strip()] = parse_brace(val)
                    ...
                    if key in name_set:
                        ...
                        if value["def"][0] not in resolve:
                            if value["def"][0] in inner_oids:
                                ...
                                cache[value["def"][0]] = inner_types[value["def"][0]]["def"]
                            else:
            ...
            if name_set:
                ...
                for key in inner_mod_id:
                    if key in name_set:
                        ...
                        mod_parent, num = inner_mod_id[key]
                ...
                if "IMPORTS" in itext:
                    ...
                    for i, impart in enumerate(imparts[1:]):
                        ...
                        if in_name_set:
                            ...
                            mib_import(in_imp_mib, in_name_set, oids, tcs, object_types, resolve)
            ...
            if extras:
                for extra in extras:
                    if extra not in resolve:
                        LOGGER.warning("Incomplete import oid resolution %s for %s",
                            extra, imp_mib)